...
function showApp(user,csrf="",forceDiary=false){if(user?.papel==="admin"&&!user?.em_visualizacao&&!forceDiary){location.href="/admin";return}if(user?.papel==="nutricionista"&&!user?.em_visualizacao&&!forceDiary){location.href="/nutritionist";return}csrfToken=csrf||"";currentUserId=String(user?.id||"");document.getElementById("authScreen").style.display="none";document.getElementById("userEmail").textContent=user?.email||"";renderViewMode(user);mealsUI();loadPersonalLists();Promise.all([loadProfile(),refresh()]).catch(e=>console.error("carregamento inicial:",e))}
...
async function bootstrap(){const j=await fetch("/api/me").then(r=>r.json());const forceDiary=new URLSearchParams(location.search).get("diario")==="1";if(j.authenticated)showApp(j.user,j.csrf,forceDiary);else document.getElementById("authScreen").style.display="flex"}
