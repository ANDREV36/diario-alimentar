...
APP_VERSION = "V41 · Prato e base estimada"
...
def find_plate_food(name, user_id):
    ...
    pc = ddb()
    try:
        row = pc.execute("SELECT id,nome FROM alimentos_usuario WHERE usuario_id=? AND LOWER(nome)=LOWER(?)", (user_id, query)).fetchone()
        if row:
            return {"id": -int(row["id"]), "nome": row["nome"]}
        row = pc.execute("SELECT id,nome FROM alimentos_usuario WHERE usuario_id=? AND LOWER(nome) LIKE LOWER(?) ORDER BY LENGTH(nome) LIMIT 1", (user_id, "%" + query + "%")).fetchone()
        if row:
            return {"id": -int(row["id"]), "nome": row["nome"]}
    finally:
        pc.close()
    nc = ndb()
    try:
        row = nc.execute("SELECT id,nome FROM alimentos WHERE LOWER(nome)=LOWER(?)", (query,)).fetchone()
        if not row:
            row = nc.execute("SELECT id,nome FROM alimentos WHERE LOWER(nome) LIKE LOWER(?) ORDER BY LENGTH(nome) LIMIT 1", ("%" + query + "%",)).fetchone()
        return {"id": int(row["id"]), "nome": row["nome"]} if row else None
    finally:
        nc.close()
...
def normalize_estimated_nutrients(raw):
    ...
    if values["energia_kcal"] <= 0 and (values["proteina_g"] + values["carboidrato_g"] + values["lipidios_g"]) <= 0:
        return None
    return values
...
<title>V41 - Diário Alimentar · Prato e base estimada</title>
<style>
...
html{
  ...
  background-image:
    ...
    url("/a_wide_high_resolution_panoramic_landscape_photo.png?v=41")!important;
...
async function confirmPlateItems(){
  const items=plateState.items.map(x=>({alimento_id:Number.isFinite(Number(x.alimento_id))?Number(x.alimento_id):null,alimento_nome:x.alimento_nome,quantidade_g:Number(x.quantidade_g),nutrientes_estimados:x.nutrientes_estimados||null,confianca:Number(x.confianca)||0}));
  ...
  try{await api("/api/consume_batch",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:day.value,refeicao:plateState.meal||meal,items})});closePlateReview();document.getElementById("platePhotoStatus").textContent="Prato adicionado em "+(plateState.meal||meal)+".";await refresh()}catch(e){alert("Não foi possível adicionar o prato: "+e.message);if(btn){btn.disabled=false;btn.textContent="✓ CONFIRMAR E ADICIONAR"}}
}
    ...
    def do_POST(self):
        ...
        if self.path=="/api/analyze_plate":
            try:
                ...
                prompt="Analise esta fotografia de um prato pronto. Retorne SOMENTE JSON válido com exatamente esta estrutura: "+json.dumps(schema,ensure_ascii=False)+". Liste cada alimento visível separadamente, usando nomes brasileiros comuns que possam ser buscados numa base nutricional, como arroz cozido, feijão cozido, frango grelhado, alface ou tomate. Estime a quantidade em gramas e colheres de sopa quando fizer sentido. Para cada item, estime nutrientes por 100 g apenas como referência aproximada, nunca como valor de rótulo. Não invente ingredientes, óleo, molhos ou alimentos não visíveis. Se a imagem estiver incerta, mantenha a melhor estimativa com confiança baixa."
                client=OpenAI(**client_args)
                resp=client.chat.completions.create(model=VISION_MODEL,messages=[{"role":"user","content":[{"type":"text","text":prompt},{"type":"image_url","image_url":{"url":image,"detail":"high"}}]}],response_format={"type":"json_object"},max_tokens=1200)
        ...
        if self.path=="/api/consume_batch":
            ...
            try:
                ...
                for item in items:
                    ...
                    if raw_id is not None:
                        ...
                        if key in created:
                            ...
                            existing=c.execute("SELECT id,nome FROM alimentos_usuario WHERE usuario_id=? AND LOWER(nome)=LOWER(?) LIMIT 1",(self.user["id"],name)).fetchone()
                            if existing:
                ...
                c.commit();self.js({"ok":True,"count":len(valid)})
            except Exception as e:
                if c:c.rollback()
                self.js({"error":str(e)},400)
            finally:
